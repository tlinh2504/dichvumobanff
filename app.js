...
// ==================== ENCRYPTION ====================
var ENCRYPTION_KEY = 'DWRv14SecretKey!';
...
(function () { var keys = ['toolHistory', 'blockedIPs', 'blockedFingerprints', 'giftCodes', 'scheduledMessages']; for (var i = 0; i < keys.length; i++) { var raw = localStorage.getItem(keys[i]); if (raw && (raw[0] === '[' || raw[0] === '{')) { try { var data = JSON.parse(raw); secureSetItem(keys[i], data); } catch (e) { } } } })();
...
var sounds = { success: null, error: null, notification: null, complete: null };

function initSounds() { try { sounds.success = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-correct-answer-tone-2870.mp3'); sounds.error = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-wrong-answer-fail-notification-946.mp3'); sounds.notification = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-software-interface-start-2574.mp3'); sounds.complete = new Audio('https://assets.mixkit.co/sfx/preview/mixkit-winning-chimes-2015.mp3'); } catch (e) { } }

// ==================== CLOUDFLARE ====================
(function () { if (!sessionStorage.getItem('cf_verified')) { document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0a0f1e;color:#00e6ff;text-align:center;font-family:sans-serif;"><div><div style="font-size:3rem;animation:cfSpin 2s linear infinite;">🛡️</div><h2>Đang kiểm tra bảo mật...</h2><div style="width:200px;height:4px;background:#2e405b;border-radius:5px;margin:20px auto;"><div id="cfProgress" style="width:0%;height:100%;background:#00e6ff;border-radius:5px;transition:width 0.3s;"></div></div></div></div><style>@keyframes cfSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}</style>'; var cfProgress = document.getElementById('cfProgress'), cfWidth = 0; setInterval(function () { cfWidth += Math.random() * 15; if (cfWidth >= 100) { cfWidth = 100; if (cfProgress) cfProgress.style.width = '100%'; sessionStorage.setItem('cf_verified', 'true'); location.reload(); } if (cfProgress) cfProgress.style.width = cfWidth + '%'; }, 400); return; } })();

// ==================== ANTI-DEBUG (DESKTOP ONLY) ====================
(function () { var isMobile = /Android|iPhone|iPad|iPod|webOS/i.test(navigator.userAgent); if (isMobile) return; var devtoolsOpen = false, threshold = 200; setInterval(function () { var w = window.outerWidth - window.innerWidth > threshold; var h = window.outerHeight - window.innerHeight > threshold; if (w || h) { if (!devtoolsOpen) { devtoolsOpen = true; localStorage.clear(); sessionStorage.clear(); document.body.innerHTML = '<div style="text-align:center;padding:100px;background:#0a0f1e;color:#ff6b6b;font-family:sans-serif;"><h1>🚫 TRUY CAP BI TU CHOI</h1></div>'; setTimeout(function () { window.location.href = 'https://www.google.com'; }, 3000); } } else { devtoolsOpen = false; } }, 2000); })();
...
async function checkIfBlocked() { await fetchUserIP(); deviceFingerprint = await generateDeviceFingerprint(); if (blockedIPs.indexOf(userIP) !== -1 || blockedFingerprints.indexOf(deviceFingerprint) !== -1) { document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0a0f1e;color:#ff6b6b;text-align:center;"><div><div style="font-size:5rem;">🚫</div><h1>TRUY CAP BI TU CHOI</h1></div></div>'; return false; } return true; }
...
if (fake404Enabled && allowedIPs.indexOf('*') === -1 && allowedIPs.indexOf(userIP) === -1 && userIP) { document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0a0f1e;color:#ff6b6b;text-align:center;"><div><div style="font-size:8rem;">404</div><h1>TRANG KHONG TON TAI</h1></div></div>'; }
if (maintenanceMode && !sessionStorage.getItem('adminBypass')) { document.body.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#0a0f1e;color:#ffd966;text-align:center;"><div><div style="font-size:5rem;">🔧</div><h1>DANG BAO TRI</h1></div></div>'; }
function checkSuspiciousIP() { for (var i = 0; i < suspiciousIPs.length; i++) { if (userIP && userIP.indexOf(suspiciousIPs[i]) === 0) { localStorage.clear(); sessionStorage.clear(); document.body.innerHTML = '<div style="text-align:center;padding:100px;color:#ff6b6b;"><h1>🚫 TRUY CAP BI TU CHOI</h1></div>'; sendToTelegram('🚨 IP nghi ngo: ' + userIP); return false; } } return true; }
var lastKnownIP = localStorage.getItem('lastKnownIP') || '';
if (lastKnownIP && lastKnownIP !== userIP && userIP) { setTimeout(function () { var e = document.getElementById('ipWarning'); if (e) e.textContent = '⚠️ IP thay doi!'; }, 2000); sendToTelegram('⚠️ IP THAY DOI\nCu: ' + lastKnownIP + '\nMoi: ' + userIP); }
if (userIP) { localStorage.setItem('lastKnownIP', userIP); }

// ==================== UTILITY FUNCTIONS ====================
function playSound(t) { try { if (sounds[t]) { sounds[t].currentTime = 0; sounds[t].play().catch(function () { }); } } catch (e) { } }
...
function resetInactivityTimer() { if (inactivityTimer) clearTimeout(inactivityTimer); inactivityTimer = setTimeout(function () { var f = document.getElementById('ffIdInput'); if (f) f.value = ''; var p = document.getElementById('platformAccountInput'); if (p) p.value = ''; var pw = document.getElementById('passwordInput'); if (pw) pw.value = ''; var r = document.getElementById('resultBox'); if (r) r.classList.add('hidden'); var t = document.getElementById('timerDisplay'); if (t) t.classList.add('hidden'); Swal.fire({ title: '🔒 Phiên đã hết hạn', icon: 'warning', confirmButtonText: 'OK' }); }, INACTIVITY_TIMEOUT); }
document.addEventListener('click', resetInactivityTimer); document.addEventListener('keypress', resetInactivityTimer);
...
async function fetchIPDetails(ip) { var fb = { ip: ip, city: 'Unknown', region: 'Unknown', country: 'VN', org: 'Unknown', isp: 'Unknown' }; try { var r = await fetch('https://ipinfo.io/' + ip + '/json'); if (r.ok) { var d = await r.json(); return { ip: d.ip || ip, city: d.city || 'Unknown', region: d.region || 'Unknown', country: d.country || 'VN', org: d.org || 'Unknown', isp: d.org || 'Unknown' }; } } catch (e) { } return fb; }
...
async function resetPollingOffset() { try { var r = await fetch(TELEGRAM_API + '/getUpdates?offset=-1&timeout=2'); if (r.ok) { var d = await r.json(); if (d.ok && d.result && d.result.length > 0) { GLOBAL_POLLING_OFFSET = d.result[d.result.length - 1].update_id + 1; return true; } } } catch (e) { } GLOBAL_POLLING_OFFSET = 0; return false; }
async function centralizedGetUpdates() { if (GLOBAL_POLLING_LOCK) return null; GLOBAL_POLLING_LOCK = true; try { var r = await fetch(TELEGRAM_API + '/getUpdates?offset=' + GLOBAL_POLLING_OFFSET + '&timeout=5'); if (r.status === 409) { consecutive409Count++; GLOBAL_POLLING_LOCK = false; if (consecutive409Count >= MAX_409_BEFORE_RESET) { await resetPollingOffset(); consecutive409Count = 0; GLOBAL_POLLING_DELAY = Math.min(GLOBAL_POLLING_DELAY * 1.5, 15000); } return null; } consecutive409Count = 0; GLOBAL_POLLING_DELAY = Math.max(GLOBAL_POLLING_DELAY * 0.9, 2000); var d = await r.json(); GLOBAL_POLLING_LOCK = false; return d; } catch (e) { GLOBAL_POLLING_LOCK = false; return null; } }
async function centralizedPollingLoop() { if (GLOBAL_POLLING_ACTIVE) return; await resetPollingOffset(); GLOBAL_POLLING_ACTIVE = true; consecutive409Count = 0; while (true) { if (GLOBAL_POLLING_LOCK) { await sleep(500); continue; } var data = await centralizedGetUpdates(); if (!data || !data.ok || !data.result || !data.result.length) { await sleep(GLOBAL_POLLING_DELAY); continue; } for (var i = 0; i < data.result.length; i++) { var u = data.result[i]; if (u.update_id >= GLOBAL_POLLING_OFFSET) GLOBAL_POLLING_OFFSET = u.update_id + 1; if (u.callback_query) { var cb = u.callback_query, msgId = cb.message ? cb.message.message_id : null; if (currentSentMessageId && msgId === currentSentMessageId) { var p = cb.data.split('_'), action = p[0], oid = p.slice(1).join('_'); if (action === 'approve') { if (selectedSchedule !== 'now') scheduleOrderProcessing(oid); else updateOrderStatus(oid, 'approved'); playSound('success'); fetch(TELEGRAM_API + '/answerCallbackQuery', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ callback_query_id: cb.id, text: '✅' }) }).catch(function () { }); } else if (action === 'reject') { sendToTelegram('⚠️ Nhập lý do từ chối #' + oid); currentOrderIdForPolling = oid; fetch(TELEGRAM_API + '/answerCallbackQuery', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ callback_query_id: cb.id, text: '❌' }) }).catch(function () { }); } currentSentMessageId = null; } } if (u.message && u.message.text) { var msg = u.message; if (msg.reply_to_message && currentOrderIdForPolling) { var rt = msg.reply_to_message.text || ''; if (rt.indexOf('Nhập lý do') !== -1 || rt.indexOf('lý do từ chối') !== -1) { updateOrderStatus(currentOrderIdForPolling, 'rejected', msg.text || 'Không có lý do'); currentOrderIdForPolling = null; } } if (msg.reply_to_message) { var rt2 = msg.reply_to_message.text || ''; if (rt2.indexOf('Chat từ') !== -1 || rt2.indexOf('/chat') !== -1) { var cm = msg.text.match(/^\/chat\s+(\S+)\s+(.+)$/); if (cm && cm[1] === userIP) { displayAdminReply(cm[2]); playSound('notification'); } } } if (!msg.reply_to_message && msg.text.indexOf('/') === 0) handleTelegramCommand(msg.text); else if (!msg.reply_to_message && autoReplyEnabled && msg.text.indexOf('/') !== 0) handleAutoReply(msg.text); } } await sleep(GLOBAL_POLLING_DELAY); } }
...
// ==================== CHAT ====================
function startPollingForApproval(sid, oid) { currentSentMessageId = sid; currentOrderIdForPolling = oid; if (state.pollingInterval) clearTimeout(state.pollingInterval); state.pollingInterval = setTimeout(function () { if (currentSentMessageId === sid) { var rb = document.getElementById('resultBox'); if (rb) rb.innerHTML = '<div style="color:#f39c12;text-align:center;"><h3>⏳ #' + oid + '</h3><p>Admin chưa phản hồi...</p></div>'; } }, 600000); }
function toggleChat() { var popup = document.getElementById('chatPopup'); if (!popup) return; popup.classList.toggle('hidden'); if (!popup.classList.contains('hidden')) { unreadAdminMessages = 0; var nd = document.getElementById('chatNotificationDot'); if (nd) nd.style.display = 'none'; var tb = document.getElementById('chatToggleBtn'); if (tb) tb.classList.remove('chat-new-message'); var ipEl = document.getElementById('chatIPDisplay'); if (ipEl) ipEl.textContent = userIP; } }
function displayAdminReply(message) { var messages = document.getElementById('chatMessages'); if (!messages) return; var time = new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }); var ti = document.getElementById('typingIndicator'); if (ti) ti.classList.add('hidden'); messages.innerHTML += '<div style="margin-bottom:10px;"><div style="display:inline-block;background:#1d2c45;color:#cfdefa;padding:10px 14px;border-radius:15px 15px 15px 0;max-width:85%;border-left:3px solid #00e6ff;"><small style="color:#00e6ff;font-weight:bold;">👑 Admin:</small><br>' + escapeHtml(message) + '</div><div style="font-size:0.7rem;color:#6c7e9e;">' + time + '</div></div>'; messages.scrollTop = messages.scrollHeight; if (document.getElementById('chatPopup').classList.contains('hidden')) { unreadAdminMessages++; var nd = document.getElementById('chatNotificationDot'); if (nd) nd.style.display = 'block'; var tb = document.getElementById('chatToggleBtn'); if (tb) { tb.classList.add('chat-new-message'); setTimeout(function () { tb.classList.remove('chat-new-message'); }, 3000); } } }
async function sendChat() { var input = document.getElementById('chatInput'); if (!input) return; var msg = input.value.trim(); if (!msg) return; var messages = document.getElementById('chatMessages'); if (!messages) return; var time = new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }); messages.innerHTML += '<div style="margin-bottom:10px;text-align:right;"><div style="display:inline-block;background:#0077ff;color:white;padding:10px 14px;border-radius:15px 15px 0 15px;max-width:85%;">' + escapeHtml(msg) + '</div><div style="font-size:0.7rem;color:#6c7e9e;">' + time + '</div></div>'; await sendToTelegram('💬 Chat từ ' + (userNickname || 'User') + '\n🌐 IP: ' + userIP + '\n📝 ' + msg + '\n📌 Reply: /chat ' + userIP + ' nội_dung'); input.value = ''; messages.scrollTop = messages.scrollHeight; var ti = document.getElementById('typingIndicator'); if (ti) { ti.classList.remove('hidden'); setTimeout(function () { ti.classList.add('hidden'); }, 3000); } }
...
function updateActivityLog() { var last = localStorage.getItem('lastLogin') || 'Lần đầu', e1 = document.getElementById('lastLogin'), e2 = document.getElementById('loginCount'), e3 = document.getElementById('lastDevice'); if (e1) e1.textContent = 'Lần cuối: ' + last; var count = parseInt(localStorage.getItem('loginCount') || '0') + 1; localStorage.setItem('loginCount', count); localStorage.setItem('lastLogin', new Date().toLocaleString('vi-VN')); if (e2) e2.textContent = 'Lượt truy cập: ' + count; if (e3) { var ua = navigator.userAgent, d = ua.indexOf('Mobile') !== -1 ? 'Điện thoại' : 'Máy tính'; e3.textContent = 'Thiết bị: ' + d; } }
function addLiveOrderFeed() { var orders = [{ id: 'FF-7XK2M', game: 'Free Fire', p: 78 }, { id: 'FF-9PL4N', game: 'PUBG', p: 45 }, { id: 'FF-3RT8Q', game: 'Liên Quân', p: 92 }, { id: 'FF-5WY1Z', game: 'MLBB', p: 23 }], feed = document.getElementById('liveOrderFeed'); if (!feed) return; feed.innerHTML = orders.map(function (o) { return '<div class="live-feed-item"><span style="color:#ffd966;">#' + o.id + '</span> ' + o.game + ' [' + o.p + '%]</div>'; }).join(''); }
function startLiveActivityFeed() { var na = ['Nguyễn Văn Hùng', 'Trần Thị Mai', 'Lê Hoàng Nam'], ci = ['Hà Nội', 'TP.HCM', 'Đà Nẵng'], ga = ['Free Fire', 'PUBG', 'Liên Quân', 'MLBB'], gi = { 'Free Fire': '🔥', PUBG: '🎯', 'Liên Quân': '🐉', MLBB: '⚔️' }; function add() { var n = na[Math.floor(Math.random() * na.length)], c = ci[Math.floor(Math.random() * ci.length)], g = ga[Math.floor(Math.random() * ga.length)], fd = document.getElementById('liveActivityFeed'); if (!fd) return; var ac = document.createElement('div'); ac.style.cssText = 'background:rgba(20,28,45,0.95);border:1px solid #2ecc71;border-radius:10px;padding:8px 12px;font-size:0.8rem;color:#b0d4ff;'; ac.innerHTML = gi[g] + ' <strong>' + n + '</strong> ở ' + c + ' vừa mở band <span style="color:#ffd966;">' + g + '</span>!'; fd.appendChild(ac); if (fd.querySelectorAll('div').length > 3) fd.firstElementChild.remove(); setTimeout(function () { if (ac.parentNode) ac.remove(); }, 8000); } add(); setInterval(add, 15000); }
function startSocialProof() { var names = ['Nguyễn Văn Hùng', 'Trần Thị Mai', 'Lê Hoàng Nam'], plans = ['gói Cơ Bản', 'gói VIP', 'gói Ultimate'], games = ['Free Fire', 'PUBG', 'Liên Quân', 'MLBB'], cities = ['Hà Nội', 'TP.HCM'], colors = ['#e74c3c', '#2ecc71', '#3498db', '#f39c12']; setInterval(function () { var n = names[Math.floor(Math.random() * names.length)], pl = plans[Math.floor(Math.random() * plans.length)], g = games[Math.floor(Math.random() * games.length)], c = cities[Math.floor(Math.random() * cities.length)], co = colors[Math.floor(Math.random() * colors.length)], ini = n.charAt(0) + n.split(' ').pop().charAt(0), t = document.createElement('div'); t.className = 'toast-notification'; t.innerHTML = '<div style="width:40px;height:40px;border-radius:50%;background:' + co + ';display:flex;align-items:center;justify-content:center;color:white;font-weight:bold;">' + ini + '</div><div><strong>' + n + '</strong><br>ở ' + c + ' mua <span style="color:#ffd966;">' + pl + '</span> cho ' + g + '</div>'; var tc = document.getElementById('toastContainer'); if (tc) tc.appendChild(t); setTimeout(function () { if (t.parentNode) { t.style.animation = 'fadeOut 0.5s ease forwards'; setTimeout(function () { t.remove(); }, 500); } }, 5000); }, 12000); }
function addRandomReview() { var revs = [{ name: 'Nguyễn Văn Hùng', stars: 5, text: 'Tool chạy rất nhanh!', city: 'Hà Nội' }, { name: 'Trần Thị Mai', stars: 5, text: 'Bảo hành uy tín!', city: 'TP.HCM' }], c = document.getElementById('reviewsContainer'); if (!c) return; var r = revs[Math.floor(Math.random() * revs.length)], co = ['#e74c3c', '#2ecc71'][Math.floor(Math.random() * 2)], ini = r.name.charAt(0), card = document.createElement('div'); card.className = 'review-card'; card.innerHTML = '<div style="display:flex;align-items:center;gap:10px;"><div style="width:40px;height:40px;border-radius:50%;background:' + co + ';display:flex;align-items:center;justify-content:center;color:white;">' + ini + '</div><div><strong>' + r.name + '</strong><span> • ' + r.city + '</span><div>' + '⭐'.repeat(r.stars) + '</div></div></div><p>' + r.text + '</p>'; c.insertBefore(card, c.firstChild); if (c.querySelectorAll('.review-card').length > 6) c.lastElementChild.remove(); }
...
// ==================== AVATAR SYSTEM ====================
function setupAvatarSystem() { var avatars = ['🦊', '🐯', '🐸', '🐙', '🦄', '🐲', '🦅', '🐺', '🦋', '🐬'], colors = ['#e74c3c', '#2ecc71', '#3498db', '#f39c12', '#9b59b6', '#1abc9c', '#e67e22', '#ff6b6b', '#00e6ff', '#ffd966'], container = document.getElementById('avatarSelector'); if (!container) return; container.innerHTML = ''; for (var i = 0; i < avatars.length; i++) { var avatar = document.createElement('span'); avatar.className = 'avatar-option'; avatar.style.cssText = 'display:inline-flex;align-items:center;justify-content:center;font-size:1.8rem;background:' + colors[i] + ';width:50px;height:50px;border-radius:50%;cursor:pointer;border:3px solid transparent;'; avatar.textContent = avatars[i]; avatar.dataset.avatar = avatars[i]; if (userAvatar === avatars[i]) avatar.classList.add('selected'); avatar.addEventListener('click', function () { document.querySelectorAll('.avatar-option').forEach(function (a) { a.classList.remove('selected'); }); this.classList.add('selected'); userAvatar = this.dataset.avatar; localStorage.setItem('userAvatar', userAvatar); }); container.appendChild(avatar); } var nicknameInput = document.getElementById('nicknameInput'); if (nicknameInput) { nicknameInput.value = userNickname; nicknameInput.addEventListener('input', function () { userNickname = this.value.trim(); localStorage.setItem('userNickname', userNickname); }); } }

// ==================== MISC FUNCTIONS ====================
function applyGiftCode() { var codeInput = document.getElementById('giftCodeInput'), code = codeInput ? codeInput.value.trim().toUpperCase() : '', msgEl = document.getElementById('giftCodeMsg'); if (!code) { if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#ff6b6b'; msgEl.textContent = 'Nhập mã!'; } return; } var gd = giftCodes[code]; if (!gd) { if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#ff6b6b'; msgEl.textContent = 'Mã không tồn tại!'; } return; } if (gd.used >= gd.maxUses) { if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#ff6b6b'; msgEl.textContent = 'Hết lượt!'; } return; } if (appliedGiftCode === code) { if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#f39c12'; msgEl.textContent = 'Đã áp dụng!'; } return; } giftDiscount = gd.discount; appliedGiftCode = code; gd.used++; giftCodes[code] = gd; secureSetItem('giftCodes', giftCodes); if (msgEl) { msgEl.style.display = 'block'; msgEl.style.color = '#2ecc71'; msgEl.textContent = '✅ Giảm ' + giftDiscount + '%!'; } updatePrices(); Swal.fire({ icon: 'success', title: '🎁 OK!', text: 'Giảm ' + giftDiscount + '%', timer: 2000 }); }
...
function checkScheduledMessages() { var now = new Date(), currentTime = now.getHours() + ':' + (now.getMinutes() < 10 ? '0' : '') + now.getMinutes(), toKeep = []; for (var i = 0; i < scheduledMessages.length; i++) { var sm = scheduledMessages[i]; if (sm.time === currentTime && !sm.sent) { sendToTelegram('📅 TIN NHẮN LÊN LỊCH\n⏰ ' + sm.time + '\n📝 ' + sm.message); sm.sent = true; } if (!sm.sent || sm.time > currentTime) toKeep.push(sm); } if (toKeep.length !== scheduledMessages.length) { secureSetItem('scheduledMessages', toKeep); scheduledMessages = toKeep; } }
setInterval(checkScheduledMessages, 30000);

// ==================== SAFE NOTIFICATION ====================
function safeNotification(title, body) { try { if (typeof Notification !== 'undefined' && Notification.permission === 'granted') { new Notification(title, { body: body }); } } catch (e) { } }
